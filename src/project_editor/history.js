...
export class History {
	...
	restoreState() {
		...
		if (q.length === 1) {
			...
			let baseItem = this.initialProject.getItem(editor.selectedItemID);
			// Selecting an uncreated character adds a blank glyph after the project
			// was opened. Its creation snapshot is the baseline for that character.
			if (!baseItem && q[0].wholeProjectSave) {
				baseItem = q[0].itemState.getItem(editor.selectedItemID);
			}
