...
export class ProjectEditor {
	...
	initializeHistory(project) {
		...
		this.history.initialProject = new GlyphrStudioProject(project.save());
	}
	...
	makeAutoFitView(rect = false) {
		...
		*/
		...
		newX += canvasPadding;
		// rect may describe a gap that starts partway across the canvas -
		// see getEditCanvasWrapperBounds.
		newX += rect.offsetX || 0;
		// log(`newX: ${newX}`);
	...
	autoFitView() {
		...
		if (bounds) {
			...
			/*
				The view setter clears `default` on every write - including this
				one. That meant a view could only ever be auto-fitted once: after
				the first fit the flag was gone, so resizing the window, dragging
				a panel or hiding the panels could never re-fit, and the glyph
				stayed wherever the old geometry had put it.

				Re-marking it here keeps the flag meaning what the rest of the
				code assumes: "this view is still an untouched auto-fit". A real
				user zoom or pan goes through the setter and clears it, which is
				what stops us from fighting someone who has framed their own view.
			*/
			const id = this.selectedItemID;
			if (this._views[id]) this._views[id].default = true;
	...
	/**
	 * The area a glyph should be fitted into.
	 *
	 * The canvas runs edge to edge behind the floating panels, so its own
	 * rectangle is not the space the user can actually see. Auto-fitting to it
	 * would centre every glyph underneath the left or right panel. What is
	 * returned instead is the clear gap between whatever panels are on screen.
	 *
	 * @returns {Object | false} - a rect-like {width, height}, or false before
	 *                             the DOM is ready
	 */
	getEditCanvasWrapperBounds() {
		const wrapper = document.querySelector('.editor-page__edit-canvas-wrapper');
		...
		const leftPanel = document.querySelector('.editor-page__left-area');
		const rightPanel = document.querySelector('.editor-page__right-area');

		// Only count a panel that is actually laid out - the right one is
		// removed entirely at narrow widths.
		if (leftPanel?.getBoundingClientRect().width) {
			left = Math.max(left, leftPanel.getBoundingClientRect().right);
		}
		if (rightPanel?.getBoundingClientRect().width) {
			right = Math.min(right, rightPanel.getBoundingClientRect().left);
		}

		const visibleWidth = right - left;

		// If the panels have eaten everything, fall back to the full canvas
		// rather than trying to fit a glyph into a negative width.
		if (visibleWidth < 120) return canvasRect;

		return {
			width: visibleWidth,
			height: canvasRect.height,
			// The view is expressed in canvas-local coordinates, so the gap
			// carries how far into the canvas it begins.
			offsetX: left - canvasRect.left,
			left: left,
			right: right,
			top: canvasRect.top,
