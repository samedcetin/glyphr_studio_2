...
export class Path extends GlyphElement {
	 ...
	 * @param {Boolean =} arg.isVisible - draw this shape, or keep it out of the way while editing
	 * @param {Boolean =} arg.isLayerLocked - protect this shape from being selected on the canvas
	...
	constructor({
		...
		/*
			Layer state, not geometry. isVisible keeps a shape out of the drawing
			without deleting it - construction guides, alternates being compared -
			and isLayerLocked keeps a finished shape from being grabbed by
			accident on the canvas.

			Named isLayerLocked rather than isLocked because isLocked() is
			already a method here, for the per-property transform locks.
		*/
		this.isVisible = isVisible;
		this.isLayerLocked = isLayerLocked;
		this.parent = parent;
		this.link = false;
	...
	save(verbose = false) {
		...
		// Written only when off the default, matching the locks above.
		if (this.isVisible === false) re.isVisible = false;
		if (this.isLayerLocked) re.isLayerLocked = true;
