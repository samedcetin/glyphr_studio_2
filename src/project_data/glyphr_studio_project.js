...
export class GlyphrStudioProject {
		...
		this.settings = {
			...
			app: {
				...
				*/
				...
				/*
					Pixel font mode. Written out here rather than imported from
					the pixel grid code: this constructor runs while modules
					are still loading, and an imported binding is not
					guaranteed to be initialised yet. `defaultPixelMode` in
					pixel_grid.js mirrors this, and a test holds the two
					together.

					It has to be listed at all because `merge` below drops any
					saved key the template does not already know about.
				*/
				pixelMode: {
					enabled: false,
					pixelsPerEm: 16,
	...
	getItemName(id, forceLongName = false) {
		...
		if (id.startsWith('glyph-')) {
			/*
				A name someone chose beats the one Unicode would have supplied.
				This matters in the Private Use Area, where the Unicode name is
				only ever the code point written out - so an icon called
				'heart' would otherwise show up everywhere as 'U+E000'.
			*/
			const namedItem = this.glyphs ? this.glyphs[id] : false;
			if (namedItem && namedItem._name && namedItem._name !== namedItem.derivedName) {
				return namedItem._name;
			}
