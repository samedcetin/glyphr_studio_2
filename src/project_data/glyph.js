...
export class Glyph extends GlyphElement {
	 ...
	 * @param {Array =} args.anchors - named attachment points, for composing accented characters
	 * @param {String =} args.leftSideBearingKey - metric key for the left sidebearing, like '=n'
	 * @param {String =} args.rightSideBearingKey - metric key for the right sidebearing, like '=|o'
	 */
	constructor({
		id = '',
		parent = false,
	...
	save(verbose = false) {
		...
		/*
			A name is only worth saving when someone chose it. Components have
			always had one; every other item type derives its name from its id,
			except when it has been renamed - which is how a Private Use Area
			glyph carries an icon name like 'heart-filled'.
		*/
		if (this.objType === 'Component') re.name = this.name;
		else if (this._name && this._name !== this.derivedName) re.name = this._name;
		...
		// Only written when there are any, so a project that never touches
		// anchors gains nothing in its save file.
		if (this.anchors.length) re.anchors = this.anchors.map((anchor) => anchor.save());

		// The key is what was meant; the sidebearing it produces is derived
	...
	/**
	 * The shapes that make up the drawing.
	 *
	 * A shape hidden from the Layers panel is still part of the glyph - it is
	 * saved, it can be shown again - but it is not part of what this glyph
	 * currently *is*. Everything that answers "what does this glyph look
	 * like" goes through here: rendering, the SVG path data that feeds export
	 * and thumbnails, and the bounding box.
	 *
	 * Keeping all of those on one definition is what stops a hidden path from
	 * being excluded from the outline while still stretching the bounding box.
	 *
	 * @returns {Array}
	 */
	get visibleShapes() {
		return this.shapes.filter((shape) => shape.isVisible !== false);
	}
	...
	/**
	 * set anchors
	 * @param {Array} newAnchors - anchors or plain saved objects
	 */
	set anchors(newAnchors) {
		this._anchors = (newAnchors || []).map((anchor) =>
			anchor instanceof Anchor ? anchor : new Anchor(anchor)
		);
	}

	/**
	 * Looks up one anchor by name.
	 * @param {String} name - anchor name
	 * @returns {Anchor | false}
	 */
	getAnchor(name) {
		return this.anchors.find((anchor) => anchor.name === name) || false;
	}

	/**
	 * Adds an anchor, or moves the one that already has this name.
	 *
	 * Names have to be unique within a glyph: two anchors called `top` would
	 * make attachment ambiguous, and whichever one won would do so silently.
	 *
	 * @param {String} name - anchor name
	 ...
	 */
	setAnchor(name, x, y) {
		const anchor = new Anchor({ name: name, x: x, y: y });
		const existing = this.getAnchor(anchor.name);
		...
		this._anchors = [...this.anchors, anchor];
		this.changed();
		return anchor;
	}
	...
	removeAnchor(name) {
		...
		this._anchors = this.anchors.filter((anchor) => anchor.name !== name);
		if (this.anchors.length === before) return false;
	...
	/**
	 * The name this item would have if nobody had named it.
	 *
	 * Kept apart from `name` so that saving can tell a name someone chose from
	 * one that was worked out from the id. Without that distinction a custom
	 * name cannot survive a save, which matters most in the Private Use Area:
	 * Unicode has no names there, so an icon's name is the only thing
	 * identifying it.
	 *
	 * @returns {String}
	 */
	get derivedName() {
		if (!this.id) return '[no id]';

		if (this.id.startsWith('liga-')) {
			const suffixArr = remove(this.id, 'liga-').split('-');
			let name = 'Ligature ';
		...
		if (this.id.startsWith('comp-')) return `Component ${remove(this.id, 'comp-')}`;
		if (this.id.startsWith('glyph-')) return getUnicodeName(remove(this.id, 'glyph-'));
	...
	recalculateGlyphMaxes() {
		...
		const drawnShapes = this.visibleShapes;
		if (drawnShapes.length > 0) {
			// log('... has shapes, calling getOverallMaxes');
			overallMaxes = getOverallMaxes(drawnShapes.map((item) => item.maxes));
		}
