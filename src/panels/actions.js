...
import { getUnitsPerPixel, isPixelModeOn } from '../pixel_font/pixel_grid.js';
import { pixelateGlyph } from '../pixel_font/pixelate.js';
import {
	...
	*/
	...
	if (name === 'glyphActions') {
		actionData = [
			...
			{
				iconName: 'metricKeys',
				title: `Update all metric keys\nRe-spaces every glyph whose sidebearings are keyed to another glyph.\n\nRun this after changing the spacing of a glyph that others are keyed to.`,
				onClick: updateAllMetricKeys,
			},
			{
				iconName: 'pixelPen',
				title: `Snap glyph to the pixel grid\nRedraws this glyph as whole pixels, and snaps its advance width to the grid.\n\nThis is how an imported outline becomes a pixel font. Curves do not survive it - a pixelated curve is a staircase - so it is worth checking the result before moving on.`,
				disabled: !isPixelModeOn(editor.project),
				onClick: snapSelectedItemToPixelGrid,
			},
			{
 ...
 * came out the other side, because "1 shape" where there were curves is the
 * quickest way to see that a glyph has been flattened too far.
 */
function snapSelectedItemToPixelGrid() {
	const editor = getCurrentProjectEditor();
	const item = editor.selectedItem;
	if (!item) return;

	const before = item.shapes.length;
	pixelateGlyph(item, getUnitsPerPixel(editor.project));

	editor.history.addState('Snapped this glyph to the pixel grid');
	editor.publish('currentItem', item);
	refreshPanel();

	if (!item.shapes.length) {
		showToast('Nothing landed on the grid. Try more pixels per em.');
		return;
	}
...
/**
 * Re-spaces every keyed glyph in the project.
 *
 * Reported as a count rather than silently: the whole value of a key is that
 * it moves things you are not looking at, so the number is the only sign it
 * did anything.
 */
function updateAllMetricKeys() {
	const editor = getCurrentProjectEditor();
	...
	let summary = `Re-spaced ${result.updated.length} glyph${result.updated.length === 1 ? '' : 's'}`;
	if (result.failed.length) summary += `<br>${result.failed.length} could not be resolved`;
	showToast(summary);
}
