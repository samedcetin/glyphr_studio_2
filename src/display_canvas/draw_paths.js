...
export function drawGlyph(glyph, ctx, view = { x: 0, y: 0, z: 1 }, alpha = 1, fill = '') {
	// An empty fill means 'use whatever the current theme calls ink'.
	if (!fill) fill = getCanvasColors().ink;
	// log('drawGlyph', 'start');
	// log(glyph.name);
	...
	let drewShape;
	ctx.beginPath();
	// visibleShapes, not shapes: a hidden layer is not part of the drawing.
	glyph.visibleShapes.forEach((shape) => {
