	...
	if (event.button === 0) {
		...
		switch (editor.selectedTool) {
			...
			case 'kern':
				...
				break;
			case 'pixelPen':
				ehd.currentToolHandler = editor.eventHandlers.tool_pixelPen;
				break;
...
/**
 * Whether a shape can be picked up on the canvas.
 *
 * Hidden shapes are not drawn, so there is nothing there to lasso, and
 * locked shapes are meant to be immune to a stray drag across them.
 *
 * @param {Object} shape - a Path or ComponentInstance
 * @returns {Boolean}
 */
function isShapeSelectable(shape) {
	return shape.isVisible !== false && !shape.isLayerLocked;
}
...
export function selectItemsInArea(x1, y1, x2, y2, type = 'pathPoints') {
	...
	if (type === 'pathPoints') {
		...
		editor.selectedItem.shapes.forEach((shape) => {
			if (!isShapeSelectable(shape)) return;
...
export function handleMouseWheel(event) {
	...
	/*
		Wheel and trackpad behaviour matches what designers already have in their
		hands from Figma, Illustrator and Sketch:

			wheel / two-finger scroll  ->  pan vertically
			shift + wheel              ->  pan horizontally
			ctrl or cmd + wheel        ->  zoom toward the pointer

		Before this, plain scrolling over the canvas did nothing at all and zoom
		was the only wheel gesture, which is why the canvas felt inert.

		Note on trackpads: browsers report a pinch gesture as a wheel event with
		ctrlKey set, so pinch-to-zoom falls out of the zoom branch for free.
	*/
	...
	if (isZoomGesture) {
		...
		// Scale the step by the size of the delta so a trackpad pinch feels
		// continuous instead of stepping in fixed 10% jumps.
		const magnitude = Math.min(Math.abs(event.deltaY), 50) / 50;
		const step = 1 + magnitude * 0.18;
		editor.updateViewZoom(event.deltaY < 0 ? step : 1 / step, mouse);
		return;
	}

	// Pan. deltaMode 1 is lines rather than pixels (mostly Firefox with a
	// classic mouse wheel); 16px per line is the usual approximation.
	const scale = event.deltaMode === 1 ? 16 : 1;
	let panX = event.deltaX * scale;
	let panY = event.deltaY * scale;

	// A wheel-only mouse has no deltaX, so shift redirects the vertical axis.
	...
	editor.view = { dx: view.dx - panX, dy: view.dy - panY };
	editor.publish('editCanvasView', editor.view);
}
