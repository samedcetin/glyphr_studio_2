...
import { dragAnchorTo, getAnchorAtLocation } from '../../anchors/anchor_canvas.js';
...
export class Tool_Resize {
	constructor() {
		...
		this.resizing = false;
		this.rotating = false;
		/** @type {Object | false} - the anchor being dragged */
	...
	mousedown() {
		...
		/*
			Anchors are checked before anything else. They are small targets
			that often sit right on top of an outline, so a shape underneath
			would win every time if the shape were tested first.
		*/
		this.draggingAnchor = getAnchorAtLocation(
			ehd.mousePosition.x,
			ehd.mousePosition.y,
			editor.selectedItem
		);
		...
		if (this.draggingAnchor) {
			this.historyTitle = `Moved anchor: ${this.draggingAnchor.name}`;
	...
	mousemove() {
		...
		if (this.draggingAnchor) {
			dragAnchorTo(this.draggingAnchor, ehd.mousePosition.x, ehd.mousePosition.y, view);
