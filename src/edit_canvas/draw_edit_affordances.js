...
/*
	These used to be module-level constants resolved once at import time, which
	meant they were baked in before the user could ever change theme. They are
	now re-read from the CSS token layer on every theme change.
*/
...
/**
 * Pulls the current theme's canvas colors into the module-level drawing
 * colors. Called once at import and again whenever the theme changes.
 */
function refreshAffordanceColors() {
	const colors = getCanvasColors();
	accentBlue = colors.selection || accentBlue;
	accentGreen = colors.pointSelected || accentGreen;
	accentGray = colors.metricStrong || accentGray;
	pathFill = colors.ink || pathFill;
	pointFill = colors.handle || pointFill;
}

refreshAffordanceColors();
onThemeChange(refreshAffordanceColors);
