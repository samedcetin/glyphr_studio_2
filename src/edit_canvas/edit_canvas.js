...
export class EditCanvas extends HTMLElement {
	...
	constructor(attributes = {}) {
		...
		/*
			width/height are the canvas size in CSS pixels. All drawing code works
			in this space. The backing store is separately scaled by the device
			pixel ratio in updateCanvasSize(), so nothing downstream has to know
			or care that the display is HiDPI.

			These start at a placeholder and are replaced by a real measurement as
			soon as the element is in the DOM.
		*/
		this.width = Number(this.getAttribute('width')) || 800;
		this.height = Number(this.getAttribute('height')) || 600;
		...
		this.dpr = 1;
		/** True while a paint is already scheduled for the next frame. */
		...
		// Canvas colors come from CSS tokens, so a theme flip has to repaint.
		this.unsubscribeTheme = onThemeChange(() => this.redraw('themeChange'));
	...
	connectedCallback() {
		...
		/*
			The canvas used to be a fixed 2000x2000 buffer that overflowed its
			wrapper and got clipped. Now it tracks the element's real size, which
			means resizing the window, collapsing a panel, or moving the window to
			a display with a different pixel ratio all stay sharp.
		*/
		if (window.ResizeObserver) {
			this.resizeObserver = new ResizeObserver(() => {
				if (this.updateCanvasSize()) {
					// A resize invalidates an auto-fit view, so recompute it.
					const currentEditor = getCurrentProjectEditor();
					if (currentEditor.view?.default) currentEditor.autoFitIfViewIsDefault();
					this.redraw('resizeObserver');
				}
			});
	...
	/**
	 * Matches the canvas backing store to the element's CSS size and the
	 * current device pixel ratio, then scales the context so that all drawing
	 * code can keep working in CSS pixels.
	 * @returns {Boolean} - true if anything actually changed
	 */
	updateCanvasSize() {
		const rect = this.getBoundingClientRect();
		// Before layout runs, or while hidden, the element measures zero. Keep
		// the previous size rather than collapsing the buffer to nothing.
		const cssWidth = Math.max(1, Math.round(rect.width)) || this.width;
		const cssHeight = Math.max(1, Math.round(rect.height)) || this.height;
		const dpr = window.devicePixelRatio || 1;

		if (cssWidth === this.width && cssHeight === this.height && dpr === this.dpr) {
			return false;
		}
		...
		// setTransform rather than scale: this runs repeatedly, and scale would
		// compound on every resize.
	...
	// --------------------------------------------------------------
	/**
	 * Requests a repaint on the next animation frame.
	 *
	 * Many things publish at once - a single drag can fire 'whichShapeIsSelected',
	 * 'currentPath' and the catch-all '*' subscription in the same tick. This
	 * collapses all of them into one paint per frame instead of one paint per
	 * event, which is the difference between a drag at 15fps and at 60fps.
	 *
	 * @param {String =} _caller - who is calling redraw, for debugging
	 */
	...
	/**
	 * Draws the canvas. Call redraw() instead of this - going straight here
	 * bypasses the per-frame coalescing.
	 * @param {String =} _caller - who is calling
	 */
	// eslint-disable-next-line @typescript-eslint/no-unused-vars
	paint(_caller = 'unknown') {
		// log(`EditCanvas.paint \\ ${_caller}`, 'start');
		const editor = getCurrentProjectEditor();
		...
		// Already inside an animation frame - draw now rather than queueing
		// another one, which used to add a frame of latency to every change.
		if (currentItemID.startsWith('kern-')) redrawKernEdit();
		else redrawGlyphEdit();

		// log(`EditCanvas.paint \\ ${_caller}`, 'end');

		function redrawGlyphEdit() {
			// log(`EditCanvas.redrawGlyphEdit`, 'start');
			editor.autoFitIfViewIsDefault();
			// The canvas paints its own ground so the glyph reads correctly in
			// both themes. --canvas-bg / --canvas-ink invert together in dark.
			ctx.fillStyle = getCanvasColors().bg;
			ctx.fillRect(0, 0, width, height);
			const ehd = eventHandlerData;

			// Pixel grid, under everything - it shows where edges should land,
			// so it must not cover the edges themselves.
			drawPixelGrid(ctx, project, view, width, height);
			...
			if (project.settings.app.canvasDisplayModeFilled) {
				drawGlyph(currentItem, ctx, view, 1, getCanvasColors().ink);
			} else {
			...
			// Anchors, on top of the outlines - they are furniture you place
			// against the shape, so they must not be hidden by it.
			drawAnchors(ctx, currentItem, view);
...
export function sXcX(sx, view = getCurrentProjectEditor().view) {
	...
	canvasX += sx * view.dz;
	/*
		`canvasX || view.dx` was here as a guard against NaN input. It also
		swallowed a legitimate result of exactly 0 - a point landing on the
		canvas origin came back as view.dx instead. isFinite keeps the guard
		and lets zero through.
	*/
	return isFinite(canvasX) ? canvasX : view.dx;
}
...
export function sYcY(sy, view = getCurrentProjectEditor().view) {
	let canvasY = view.dy;
	canvasY -= sy * view.dz;
	// Same zero-vs-NaN distinction as sXcX above.
	return isFinite(canvasY) ? canvasY : view.dy;
}
