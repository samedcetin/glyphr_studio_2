...
function addGlobalEventListeners() {
	// Ctrl/Cmd+K and Ctrl/Cmd+? are registered in capture phase, so they work
	// from any page and are never swallowed by the canvas or panel handlers.
	addCommandPaletteListeners();

	// if (!getGlyphrStudioApp().settings.dev.mode) {
	window.addEventListener('mouseup', (event) => {
		/*
			Ignore the secondary-button release. Browsers fire contextmenu between
			mousedown and mouseup, so without this the right-click that opens the
			canvas context menu is immediately followed by the mouseup that closes
			it again - the menu would appear and vanish within the same gesture.
		*/
		if (event.button === 2) return;

		// The open dropdown, if any - not every navigation landmark on the page.
